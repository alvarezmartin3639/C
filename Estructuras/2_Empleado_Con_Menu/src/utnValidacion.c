...
int getStringDeLetrasConEspacio(char *pResultado) {
	...
	if (pResultado != NULL) {
		...
		if (myGets(buffer, sizeof(buffer)) == 0 && esLetraConEspacio(buffer) == 0) {
			strcpy(pResultado, buffer);
			retorno = 0;
		}
...
int getPedirStringDeLetrasConEspacio(char *pArrayChar, char *mensaje, char *mensajeError,
		int reintentos) {
	...
	if (pArrayChar != NULL && mensaje != NULL && mensajeError != NULL
		...
		while (reintentos > 0) {
			...
			if (getStringDeLetrasConEspacio(cadena) == 0)
				break;
