...
int altaEmpleados(eEmpleado *empleados, int tamEmpleado, int *contId) {
	...
	if (empleados == NULL && contId == NULL && tamEmpleado < 1)
		printf("\n***Error, no se pudo ingresar al menu de crear empleado***");
		...
		do {
			...
			if ((indiceDelEmpleado = buscarIndiceValorIntEnArray(&empleados->isEmpity, tamEmpleado, 1)) == -1)
				printf("\n***Error, no existe empleado vacio***");

			if (indiceDelEmpleado == -1)
				printf("\n***Error, No hay espacio para un nuevo empleado!... se lo devolverá al menu principal***");
...
int modificarEmpleados(eEmpleado *empleados, int tamEmpleado) {
	...
	if (empleados != NULL && tamEmpleado > 0) {
		...
		do {
			...
			else if ((indiceDelEmpleado = buscarIndiceValorIntEnArray(&empleados->id, tamEmpleado, id)) == -1)
...
int eliminarEmpleados(eEmpleado *empleados, int tamEmpleado) {
	...
	if (empleados != NULL) {
		...
		do {
			...
			if ((indiceDelEmpleado = buscarIndiceValorIntEnArray(&empleados->id, tamEmpleado, id)) == -1 || empleados[indiceDelEmpleado].isEmpity == 1)
				printf("\n***Error, no existe un empleado con ese id***");
