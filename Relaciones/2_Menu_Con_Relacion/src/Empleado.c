...
int mostrarEmpleado(eEmpleado *empleado, eSector *sectores, int tamSectores, int conCabecera) {
	...
	if (empleado != NULL && sectores != NULL && tamSectores > 0) {
		...
		retorno = printf("\nSector -> %s\n", mostrarDescripcionSectorPorId(empleado->idSector, sectores, tamSectores));
...
char* mostrarDescripcionSectorPorId(int idDelSector, eSector *sectores, int tamSectores) {
	...
	if (sectores == NULL || tamSectores < 1)
		return strdup("***Error al mostrar la descripción del sector, sectores null o con tamaño menor a 0***");
...
int cargarEmpleado(eEmpleado *empleado, int idEmpleado, int idSector, char *name, char *lastName, float salary, int sector, int isEmpity) {
	...
	if (empleado != NULL && name != NULL && lastName != NULL && (isEmpity == 0 || isEmpity == 1) && idSector >= IDMIN) {
 ...
 */
int altaEmpleados(eEmpleado *empleados, eSector *sectores, int tamSectores, int tamEmpleados, int *contId, int *contEmpleadosActivos) {
	...
	if (empleados == NULL || contId == NULL || tamEmpleados < 1 || contEmpleadosActivos == NULL || sectores == NULL || tamSectores < 1)
		...
		do {
			...
			if ((indiceDelEmpleado = buscarIntEnArray(&empleados->isEmpity, tamEmpleados, 1)) == -1)
			...
			//FALTA REVISAR SI ANDA BIEN
			else if (getPedirSectorValido(nombreSector, "\nIngrese el nombre del sector: ", sectores, tamSectores,
				...
				//+1 porque retorna el indice y el id es el indice + 1
				auxEmpleado.idSector = sectores[esSectorValido(sectores, tamSectores, nombreSector)].idSector;

				limpiarPantalla();
				mostrarEmpleado(&auxEmpleado, sectores, tamSectores, 1);
...
int mostrarEmpleados(eEmpleado *empleados, eSector *sectores, int tamEmpleado, int tamSectores, int estaVacio, int conCabecera,
	...
	if (empleados != NULL && sectores != NULL && tamEmpleado > 0 && tamSectores > 0 && (estaVacio == 1 || estaVacio == 0)) {
