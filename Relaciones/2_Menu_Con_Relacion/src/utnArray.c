...
int buscarIndiceMaximoArrayInt(int array[], int tam) {
	...
	if (array != NULL && tam > 0) {
		...
		for (i = 0; i < tam; i++) {
			...
			if (valorMaximo < array[i] && array[i] != -1) {
...
//int contarStringQueCumplanCondicion(char *arrayDeDatos, int tamArrays, int *condicionEntera, int valorCondicion) {
//
//	if (arrayDeDatos != NULL && tamArrays > 0 && condicionEntera != NULL) {
//
//		for (int i = 0; i < tamArrays; i++) {
//
//			if (condicionEntera[i] == valorCondicion) {
//
//				for (int j = 0; j < i; j++) {
//
//					if (strcmp(arrayDeDatos[i], arrayDeDatos[j]) == 0)
//						break;
//
//					if(j==i){
//
