...
typedef struct {
	...
	int contEntero;
	//el indice que hace la relación entre el eArray y el array pasado por parametro formal
	int indiceRelacional;
	char infoString[55];
...
/**
 * @fn int buscarIntEnArray(int[], int, int)
 * @brief busca en el array el valorBuscado pasado por parametro y lo retorna
 *
...
int contarStringQueCumplanCondicion();
